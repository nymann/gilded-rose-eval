package com.gildedrose;

import java.util.function.DoubleSupplier;

class GildedRose {
    Item[] items;
    private final DoubleSupplier oracleRoll;

    public GildedRose(Item[] items) {
        this(items, Math::random);
    }

    public GildedRose(Item[] items, DoubleSupplier oracleRoll) {
        this.items = items;
        this.oracleRoll = oracleRoll;
    }

    public void updateQuality() {
        for (int i = 0; i < items.length; i++) {
            if (!items[i].name.equals("Aged Brie") && !items[i].name.equals("Backstage passes to a TAFKAL80ETC concert")) {
                if (items[i].quality > 0) {
                    if (!items[i].name.equals("Sulfuras, Hand of Ragnaros") && !items[i].name.equals("Oracle Stone") && !items[i].name.equals("Sealed Oracle Stone")) {
                        items[i].quality = items[i].quality - 1;
                    }
                }
            } else {
                if (items[i].quality < 50) {
                    items[i].quality = items[i].quality + 1;

                    if (items[i].name.equals("Backstage passes to a TAFKAL80ETC concert")) {
                        if (items[i].sellIn < 11) {
                            if (items[i].quality < 50) {
                                items[i].quality = items[i].quality + 1;
                            }
                        }

                        if (items[i].sellIn < 6) {
                            if (items[i].quality < 50) {
                                items[i].quality = items[i].quality + 1;
                            }
                        }
                    }
                }
            }

            if (items[i].name.equals("Sealed Oracle Stone")) {
                items[i].sellIn = items[i].sellIn + 1;
            } else if (items[i].name.equals("Oracle Stone")) {
                items[i].sellIn = items[i].sellIn + 1;
                if (items[i].sellIn % 3 == 0) {
                    items[i].quality = items[i].quality + 1;
                }
                if (items[i].sellIn % 7 == 0) {
                    double roll = oracleRoll.getAsDouble();
                    if (roll < 0.9) {
                        items[i].quality = items[i].quality + 10;
                    } else {
                        items[i].name = "Sealed Oracle Stone";
                    }
                }
            } else if (!items[i].name.equals("Sulfuras, Hand of Ragnaros")) {
                items[i].sellIn = items[i].sellIn - 1;
            }

            if (items[i].sellIn < 0) {
                if (!items[i].name.equals("Aged Brie")) {
                    if (!items[i].name.equals("Backstage passes to a TAFKAL80ETC concert")) {
                        if (items[i].quality > 0) {
                            if (!items[i].name.equals("Sulfuras, Hand of Ragnaros")) {
                                items[i].quality = items[i].quality - 1;
                            }
                        }
                    } else {
                        items[i].quality = items[i].quality - items[i].quality;
                    }
                } else {
                    if (items[i].quality < 50) {
                        items[i].quality = items[i].quality + 1;
                    }
                }
            }
        }
    }
}
